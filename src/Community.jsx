...
/* ─── PROFANITY LIST ─────────────────────────
   Common curse words — 18+ channels are exempt  */
...
function Particles() {
  ...
  useEffect(() => {
    ...
    const pts = Array.from({ length: 55 }, () => ({
      ...
      r:Math.random()*1.2+0.2, vx:(Math.random()-0.5)*0.25, vy:(Math.random()-0.5)*0.25,
      color:COLORS[Math.floor(Math.random()*COLORS.length)], alpha:Math.random()*0.5+0.1,
    }));
    const draw = () => {
...
export default function Community() {
  ...
  const [modStatus, setModStatus] = useState(null); // user's own moderation record
  ...
  const [sysMsg, setSysMsg]   = useState(null); // { type, text }

  const messagesEndRef = useRef(null);
  const textareaRef    = useRef(null);

  // ── Show onboard if no user ──
  useEffect(() => { if (!user) setShowOnboard(true); }, [user]);

  // ── Load channels + subscribe ──
  useEffect(() => {
    setChLoading(true);
  ...
  // ── Load own moderation status + subscribe ──
  useEffect(() => {
    if (!user) return;
    sb.from("moderation").select("*").eq("user_id", user.id).single()
      .then(({ data }) => setModStatus(data || null));

    const ch = sb.channel(`mod-${user.id}`)
      .on("postgres_changes",{
        event:"*", schema:"public", table:"moderation",
  ...
  const sendMessage = async (text, imgUrl) => {
    ...
    // Profanity check — only on non-18+ channels
    if (!isAdultChannel && text.trim() && containsProfanity(text)) {
      ...
      if (newWarnings >= 2) {
        // Ban
        const row = { user_id:user.id, user_name:user.name, status:"banned", warnings:newWarnings, reason:"Auto-banned: repeated profanity", updated_at:new Date().toISOString() };
        ...
        const row = { user_id:user.id, user_name:user.name, status:newStatus, warnings:newWarnings, reason:`Warning ${newWarnings}: profanity`, updated_at:new Date().toISOString() };
        await sb.from("moderation").upsert(row, { onConflict:"user_id" });
        setModStatus(row);
        setSysMsg({ type:"warn", text:`⚠ Warning ${newWarnings}/2: Profanity is not allowed here. One more and you will be banned.` });
        setTimeout(() => setSysMsg(null), 6000);
        return; // Block the message — don't send it
      }
    }
  ...
  return (
    <>
        ...
        <div className="comm-main">
            ...
            <div className="chat-header-left">
              ...
              <div>
                <p className="chat-header-title">
                  ...
                  {isAdultChannel && <span style={{marginLeft:"0.5rem",fontSize:"0.6rem",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,padding:"0.1rem 0.4rem",borderRadius:"3px",background:"rgba(255,45,122,0.15)",border:"1px solid rgba(255,45,122,0.3)",color:"var(--magenta)"}}>18+</span>}
                </p>
          ...
          <div className="chat-messages">
            {loading ? (
              ...
              if (item.type==="date") return <div key={item.id} className="msg-date-divider"><span>{item.label}</span></div>;
              const m = item.msg;
                  ...
                  <div className="msg-content">
                    ...
                    <div className="msg-reactions">
                      {Object.entries(m.reactions||{}).filter(([,v])=>v.count>0).map(([emoji,v])=>(
                        <button key={emoji} className={`reaction-btn ${v.users?.includes(user?.id)?"mine":""}`} onClick={()=>addReaction(m.id,emoji)}>{emoji} {v.count}</button>
                      ))}
          ...
          <div className="chat-input-area">
            ...
            {!isAdultChannel && <p className="input-hint">Enter to send · Shift+Enter for new line · 📎 to share images</p>}
          </div>
        </div>
      </div>
      ...
      {/* Onboarding */}
      {showOnboard && <Onboard onDone={u=>{setUser(u);lSet("rm_chat_user",u);setShowOnboard(false);}}/>}
  ...
  return (
    <div className="onboard-bg">
      <div className="onboard-box">
        ...
        <p className="onboard-sub">{isEdit?"Update your display name, avatar, or colour.":"Pick your display name and avatar to start chatting with other Rex Magnus readers."}</p>
