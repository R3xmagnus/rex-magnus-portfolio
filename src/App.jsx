...
.nav-burger{display:none;flex-direction:column;gap:5px;width:36px;height:36px;align-items:center;justify-content:center;cursor:pointer;border:1px solid var(--border);border-radius:5px;background:rgba(255,255,255,0.03);transition:border-color 0.2s}
.nav-burger:hover{border-color:var(--cyan)}
.nav-burger span{width:18px;height:1.5px;background:var(--soft);border-radius:1px}
.nav-drawer{position:fixed;top:64px;left:0;right:0;z-index:99;background:rgba(5,5,7,0.97);backdrop-filter:blur(20px);border-bottom:1px solid var(--border);padding:1.5rem 2rem;display:flex;flex-direction:column;gap:1rem;animation:slideDown 0.22s ease}
.nav-drawer a,.nav-drawer button{font-family:'Rajdhani',sans-serif;font-weight:700;font-size:1rem;letter-spacing:0.12em;text-transform:uppercase;color:var(--muted);text-decoration:none;transition:color 0.2s;background:none;border:none;cursor:pointer;text-align:left;padding:0.3rem 0}
.nav-drawer a:hover,.nav-drawer button:hover{color:var(--cyan)}
@keyframes slideDown{from{opacity:0;transform:translateY(-10px)}to{opacity:1;transform:translateY(0)}}
.book-hot-tag{position:absolute;top:0.7rem;right:0.7rem;padding:0.22rem 0.6rem;border-radius:3px;background:linear-gradient(135deg,rgba(255,184,0,0.95),rgba(255,120,0,0.9));font-family:'Rajdhani',sans-serif;font-size:0.58rem;font-weight:800;letter-spacing:0.12em;text-transform:uppercase;color:#0a0800;box-shadow:0 2px 8px rgba(255,184,0,0.5)}
...
@media(max-width:900px){
  ...
  .dash-acts{gap:0.4rem}
  .da{font-size:0.65rem;padding:0.35rem 0.6rem}
  ...
  .books-grid{grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:1.2rem}
...
@media(max-width:480px){
  ...
  .tab{white-space:nowrap;font-size:0.68rem;padding:0.6rem 0.9rem}
}
`;
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        // Books
        const { data: bData } = await sb.from("books").select("*").order("created_at", { ascending: false });
        if (bData?.length) {
          setBooks(bData.map(b => ({
            id: b.id, title: b.title, genre: b.genre, synopsis: b.synopsis,
            coverUrl: b.cover_url, links: b.links || [], createdAt: b.created_at,
          })));
        }
        // Author
  ...
  // A book is "hot" if it has at least 5 clicks AND is in the top half by clicks
  const HOT_THRESHOLD = 5;
  const isHot = (bookId) => (clicks[bookId] || 0) >= HOT_THRESHOLD;
  ...
  return (
    <>
      ...
      <nav className="nav">
        ...
        <button className="nav-burger" onClick={() => setMobNav(p => !p)}>
          <span /><span /><span />
        </button>
      </nav>
      {mobNav && (
        <div className="nav-drawer">
          {[["Books","books"],["About","about"],["Contact","contact"]].map(([lbl,id]) => (
            <a key={id} href={`#${id}`} onClick={e => { e.preventDefault(); scrollTo(id); setMobNav(false); }}>{lbl}</a>
          ))}
          <a href="/community" onClick={e => { e.preventDefault(); goTo('/community'); }} style={{ color: "var(--cyan)" }}>Community ✦</a>
          <button onClick={() => { setAdminState("login"); setPassErr(""); setPassInput(""); setMobNav(false); }} style={{ color: "var(--cyan)" }}>⚙ Admin</button>
        </div>
      )}
          ...
          {books.map((book, i) => (
            <div key={book.id} className="book-card" tabIndex={0} role="button"
              ...
              <div className="book-cover">
                ...
                {isHot(book.id) && <span className="book-hot-tag">🔥 Hot</span>}
              </div>
