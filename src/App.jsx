...
const DEFAULT_BOOKS = [
  {
    ...
    coverUrl: "",
    links: [{ label: "Amazon", url: "#" }, { label: "Goodreads", url: "#" }],
  },
  {
    id: "b2",
    ...
    coverUrl: "",
    links: [{ label: "Amazon", url: "#" }, { label: "Apple Books", url: "#" }],
  },
  {
    id: "b3",
    ...
    coverUrl: "",
    links: [{ label: "Amazon", url: "#" }, { label: "Barnes & Noble", url: "#" }],
  },
];
...
const DEFAULT_AUTHOR = {
  ...
  bio1: "Write your first bio paragraph here.",
  bio2: "A second paragraph here.",
  photoUrl: "",
...
:root{
  --black:#050507;--deep:#0a0a0f;--dark:#0f0f18;--panel:#13131e;
  --border:rgba(255,255,255,0.06);--border2:rgba(255,255,255,0.12);
  --cyan:#00d4ff;--cyan2:#00a8cc;--violet:#7b2fff;--violet2:#5a1fcc;
  --magenta:#ff2d7a;--gold:#ffb800;--text:#e8e8f0;--muted:#6b6b85;--soft:#9898b8;--r:8px;
}
...
::-webkit-scrollbar{width:4px}::-webkit-scrollbar-track{background:var(--black)}
...
.nav{position:fixed;top:0;left:0;right:0;z-index:100;display:flex;align-items:center;justify-content:space-between;padding:0 3rem;height:64px;background:rgba(5,5,7,0.85);backdrop-filter:blur(20px);border-bottom:1px solid var(--border)}
.nav::after{content:'';position:absolute;bottom:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,var(--cyan),var(--violet),transparent);opacity:0.5}
.nav-logo{font-family:'Bebas Neue',sans-serif;font-size:1.5rem;letter-spacing:0.08em;background:linear-gradient(135deg,var(--cyan),var(--violet));-webkit-background-clip:text;-webkit-text-fill-color:transparent;text-decoration:none;cursor:pointer}
.nav-links{display:flex;align-items:center;gap:2rem}
.nav-links a{font-family:'Rajdhani',sans-serif;font-weight:600;font-size:0.8rem;letter-spacing:0.14em;text-transform:uppercase;color:var(--muted);text-decoration:none;transition:color 0.2s;position:relative}
...
.nav-admin{padding:0.4rem 1.1rem;border-radius:4px;border:1px solid rgba(0,212,255,0.3);background:rgba(0,212,255,0.05);color:var(--cyan);font-family:'Rajdhani',sans-serif;font-size:0.75rem;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;cursor:pointer;transition:all 0.25s}
...
.hero-bg{position:absolute;inset:0;z-index:0;background:radial-gradient(ellipse 80% 70% at 0% 50%,rgba(123,47,255,0.14) 0%,transparent 65%),radial-gradient(ellipse 50% 40% at 100% 20%,rgba(0,212,255,0.08) 0%,transparent 60%),radial-gradient(ellipse 40% 50% at 60% 90%,rgba(255,45,122,0.06) 0%,transparent 60%),var(--black)}
.hero-scan{position:absolute;inset:0;z-index:1;pointer-events:none;background:repeating-linear-gradient(0deg,transparent,transparent 2px,rgba(0,212,255,0.008) 2px,rgba(0,212,255,0.008) 4px)}
.hero-left{position:relative;z-index:2;display:flex;flex-direction:column;justify-content:center;padding:5rem 3rem 5rem 5rem}
.hero-platform-tag{display:inline-flex;align-items:center;gap:0.55rem;padding:0.32rem 0.9rem;border-radius:4px;border:1px solid rgba(0,212,255,0.2);background:rgba(0,212,255,0.05);margin-bottom:1.6rem;width:fit-content;animation:fadeDown 0.7s ease both}
.hero-platform-tag .ptag-dot{width:5px;height:5px;border-radius:50%;background:var(--cyan);animation:blink 1.5s ease infinite;flex-shrink:0}
.hero-platform-tag span{font-family:'Rajdhani',sans-serif;font-weight:600;font-size:0.68rem;letter-spacing:0.18em;text-transform:uppercase;color:var(--cyan)}
@keyframes blink{0%,100%{opacity:1}50%{opacity:0.15}}
.hero-name{font-family:'Bebas Neue',sans-serif;font-size:clamp(3.5rem,6vw,7rem);line-height:0.92;letter-spacing:0.04em;margin-bottom:0.6rem;animation:fadeUp 0.85s ease 0.1s both}
.hero-name .nm-first{display:block;background:linear-gradient(135deg,#fff 0%,rgba(255,255,255,0.75) 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.hero-name .nm-last{display:block;background:linear-gradient(135deg,var(--cyan),var(--violet));-webkit-background-clip:text;-webkit-text-fill-color:transparent;filter:drop-shadow(0 0 30px rgba(0,212,255,0.25))}
.hero-platform-line{display:flex;align-items:center;gap:0.7rem;flex-wrap:wrap;margin-bottom:1.8rem;animation:fadeUp 0.85s ease 0.2s both}
.hero-platform-line .pl-rule{width:28px;height:1px;background:linear-gradient(90deg,var(--violet),var(--cyan));flex-shrink:0}
.hero-platform-line p{font-family:'Rajdhani',sans-serif;font-size:0.82rem;font-weight:500;color:var(--soft);letter-spacing:0.06em}
.hero-platform-line .pl-badge{padding:0.15rem 0.55rem;border-radius:3px;background:rgba(123,47,255,0.18);border:1px solid rgba(123,47,255,0.3);font-family:'Rajdhani',sans-serif;font-size:0.62rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:var(--violet);white-space:nowrap}
.hero-tagline{font-family:'Rajdhani',sans-serif;font-size:1.05rem;font-weight:400;color:var(--muted);line-height:1.75;max-width:480px;margin-bottom:2.8rem;letter-spacing:0.03em;animation:fadeUp 0.85s ease 0.3s both}
.hero-btns{display:flex;gap:1rem;flex-wrap:wrap;animation:fadeUp 0.85s ease 0.4s both;margin-bottom:3.5rem}
.btn-glow{display:inline-flex;align-items:center;gap:0.6rem;padding:0.85rem 2rem;border-radius:4px;border:none;cursor:pointer;text-decoration:none;font-family:'Rajdhani',sans-serif;font-size:0.88rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;background:linear-gradient(135deg,var(--violet),var(--cyan2));color:#fff;transition:all 0.3s;box-shadow:0 4px 30px rgba(123,47,255,0.4);position:relative;overflow:hidden}
.btn-glow::before{content:'';position:absolute;inset:0;background:linear-gradient(135deg,var(--cyan),var(--violet2));opacity:0;transition:opacity 0.3s}
.btn-glow:hover::before{opacity:1}
.btn-glow:hover{transform:translateY(-2px);box-shadow:0 8px 40px rgba(123,47,255,0.6)}
.btn-glow span{position:relative}
.btn-outline{display:inline-flex;align-items:center;gap:0.6rem;padding:0.85rem 2rem;border-radius:4px;border:1px solid var(--border2);background:rgba(255,255,255,0.03);color:var(--soft);cursor:pointer;text-decoration:none;font-family:'Rajdhani',sans-serif;font-size:0.88rem;font-weight:600;letter-spacing:0.08em;text-transform:uppercase;transition:all 0.25s}
.btn-outline:hover{border-color:var(--cyan);color:var(--cyan);background:rgba(0,212,255,0.05)}
.hero-stats{display:flex;gap:0;border:1px solid var(--border);border-radius:6px;background:rgba(255,255,255,0.02);backdrop-filter:blur(10px);overflow:hidden;animation:fadeUp 0.9s ease 0.55s both;width:fit-content}
...
.hstat-n{font-family:'Bebas Neue',sans-serif;font-size:2rem;background:linear-gradient(135deg,var(--cyan),var(--violet));-webkit-background-clip:text;-webkit-text-fill-color:transparent;line-height:1;display:block}
.hstat-l{font-family:'Rajdhani',sans-serif;font-size:0.65rem;font-weight:600;letter-spacing:0.14em;text-transform:uppercase;color:var(--muted);display:block;margin-top:0.25rem}
.hero-right{position:relative;z-index:2;display:flex;align-items:center;justify-content:center;overflow:hidden}
.hero-right-bg{position:absolute;inset:0;background:radial-gradient(ellipse 70% 80% at 50% 50%,rgba(123,47,255,0.12) 0%,transparent 70%)}
.hero-right-grid{position:absolute;inset:0;background-image:linear-gradient(rgba(123,47,255,0.06) 1px,transparent 1px),linear-gradient(90deg,rgba(0,212,255,0.04) 1px,transparent 1px);background-size:44px 44px;mask-image:radial-gradient(ellipse at center,black 40%,transparent 78%);-webkit-mask-image:radial-gradient(ellipse at center,black 40%,transparent 78%)}
.hero-book-3d{position:relative;display:flex;flex-direction:column;align-items:center;gap:2rem;z-index:2}
.hero-book-cover{width:200px;aspect-ratio:2/3;border-radius:6px;background:linear-gradient(160deg,#1e1030,#0d0d1a);border:1px solid rgba(123,47,255,0.3);box-shadow:0 40px 80px rgba(0,0,0,0.7),0 0 60px rgba(123,47,255,0.18),-8px 0 20px rgba(0,0,0,0.5);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:0.8rem;padding:1.5rem;text-align:center;position:relative;overflow:hidden;animation:floatBook 7s ease-in-out infinite}
@keyframes floatBook{0%,100%{transform:translateY(0) rotateY(-4deg)}50%{transform:translateY(-18px) rotateY(-2deg)}}
.hero-book-cover::before{content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(123,47,255,0.12) 0%,transparent 55%)}
.hero-book-cover::after{content:'';position:absolute;top:0;left:0;bottom:0;width:6px;background:linear-gradient(to bottom,var(--violet),var(--cyan));border-radius:6px 0 0 6px}
.hero-book-cover img{width:100%;height:100%;object-fit:cover;position:absolute;inset:0;border-radius:6px}
.hbc-icon{font-size:3.5rem;opacity:0.3;position:relative}
.hbc-title{font-family:'Bebas Neue',sans-serif;font-size:1rem;letter-spacing:0.08em;color:rgba(232,232,240,0.85);line-height:1.25;position:relative}
.hbc-genre{font-family:'Rajdhani',sans-serif;font-size:0.6rem;font-weight:700;letter-spacing:0.2em;text-transform:uppercase;color:var(--cyan);position:relative}
.hero-float-chip{position:absolute;padding:0.4rem 1rem;border-radius:4px;background:linear-gradient(135deg,rgba(123,47,255,0.9),rgba(0,168,204,0.9));color:white;font-family:'Rajdhani',sans-serif;font-size:0.65rem;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;box-shadow:0 4px 16px rgba(123,47,255,0.5)}
...
.hero-scroll{position:absolute;bottom:2rem;left:5rem;display:flex;flex-direction:column;align-items:center;gap:0.5rem;color:var(--muted);animation:fadeUp 1s ease 1s both;z-index:3}
...
.sec-eyebrow span{font-family:'Rajdhani',sans-serif;font-size:0.72rem;font-weight:700;letter-spacing:0.22em;text-transform:uppercase;color:var(--cyan)}
.sec-title{font-family:'Bebas Neue',sans-serif;font-size:clamp(2.5rem,5vw,4.5rem);letter-spacing:0.05em;line-height:0.95;margin-bottom:0.8rem}
...
.books-section::before{content:'';position:absolute;top:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,var(--violet),var(--cyan),transparent)}
...
.book-card::before{content:'';position:absolute;inset:-1px;border-radius:6px;background:linear-gradient(135deg,var(--cyan),var(--violet),var(--magenta));opacity:0;transition:opacity 0.4s;z-index:-1}
.book-card:hover::before{opacity:0.4}
.book-cover{aspect-ratio:2/3;border-radius:5px;overflow:hidden;position:relative;background:linear-gradient(160deg,#1a1128,#0d0d1a);box-shadow:0 20px 60px rgba(0,0,0,0.7);transition:box-shadow 0.4s}
.book-card:hover .book-cover{box-shadow:0 30px 80px rgba(0,0,0,0.8),0 0 40px rgba(123,47,255,0.2)}
.book-cover img{width:100%;height:100%;object-fit:cover;display:block}
.book-ph{width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:0.8rem;padding:1.5rem;text-align:center;background:linear-gradient(160deg,#1a1128 0%,#0d0d1a 100%);position:relative;overflow:hidden}
.book-ph::before{content:'';position:absolute;inset:0;background:repeating-linear-gradient(45deg,transparent,transparent 20px,rgba(123,47,255,0.03) 20px,rgba(123,47,255,0.03) 21px)}
...
.book-overlay{position:absolute;inset:0;border-radius:5px;background:linear-gradient(to top,rgba(5,5,7,0.97) 0%,rgba(5,5,7,0.5) 50%,transparent 100%);opacity:0;transition:opacity 0.35s;display:flex;flex-direction:column;justify-content:flex-end;padding:1.2rem}
...
.pw-success{display:flex;align-items:center;gap:0.7rem;padding:0.9rem 1.1rem;border-radius:6px;margin-top:1rem;background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.25);font-family:'Rajdhani',sans-serif;font-size:0.88rem;font-weight:600;color:#22c55e;letter-spacing:0.06em;animation:fadeIn 0.3s ease}
...
@media(max-width:900px){
  ...
  .author-photo-editor{grid-template-columns:1fr}
  .author-photo-preview{width:100%;aspect-ratio:3/2}
}
`;
    ...
    (async () => {
      try {
        ...
        if (aData) {
          setAuthor({
            name: aData.name,
            tagline: aData.tagline,
            bio1: aData.bio1,
            bio2: aData.bio2,
            photoUrl: aData.photo_url,
            email: aData.email,
        ...
        if (cData) {
          const cm = {};
          cData.forEach((c) => { cm[c.book_id] = c.count; });
          setClicks(cm);
        }
    ...
    <>
      ...
      <section className="hero" id="home">
        ...
        <div className="hero-left">
          <div className="hero-platform-tag">
            <span className="ptag-dot" />
            <span>Urban Fantasy · Male Lead Fiction</span>
          </div>
          ...
          <div className="hero-btns">
            <a className="btn-glow" href="#books" onClick={(e) => { e.preventDefault(); scrollTo("books"); }}>
              <span>Explore Books</span> <span>→</span>
            </a>
            ...
            <div className="hero-book-cover">
              {books[0]?.coverUrl ? (
                ...
                <>
                  ...
                  <span className="hbc-title">{books[0]?.title || "Iron & Shadow"}</span>
                  <span className="hbc-genre">{books[0]?.genre || "Urban Fantasy"}</span>
                </>
              )}
            </div>
          </div>
      ...
      <section className="books-section section" id="books">
        ...
        <h2 className="sec-title">
          <span className="grad">Stories Built </span>
          <span className="col">With Passion</span>
        </h2>
        ...
        <div className="about-grid">
          <div className="about-img-wrap">
            ...
            <div className="about-corner">
              <span className="about-corner-n">{books.length}</span>
              <span className="about-corner-l">Books</span>
            </div>
          </div>
          <div>
      ...
      <section className="contact-section" id="contact">
        <div className="contact-inner">
          <div className="sec-eyebrow" style={{ justifyContent: "center" }}>
            <span className="sec-eyebrow-line" /><span>Get In Touch</span>
          </div>
      ...
      {/* COMMUNITY CTA */}
      <section style={{ background: "var(--dark)", padding: "5rem 4rem", textAlign: "center", position: "relative", overflow: "hidden", borderTop: "1px solid var(--border)" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: "580px", margin: "0 auto" }}>
          <div className="sec-eyebrow" style={{ justifyContent: "center" }}>
            <span className="sec-eyebrow-line" /><span>Join the Conversation</span>
          </div>
          <h2 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: "clamp(2rem,4vw,3.5rem)", letterSpacing: "0.05em", color: "#fff", margin: "0.5rem 0 1rem", lineHeight: 1 }}>
            Reader{" "}
            <span style={{ background: "linear-gradient(135deg,var(--cyan),var(--violet))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Community</span>
          </h2>
          ...
          <a href="/community" onClick={(e) => { e.preventDefault(); goTo("/community"); }} className="btn-glow" style={{ textDecoration: "none" }}>
                ...
                <>
                  ...
                  <div className="modal-links">
                    {selectedBook.links.map((l, i) => (
                      <a key={i} href={l.url} target="_blank" rel="noopener noreferrer" className={`mlink ${i === 0 ? "mlink-p" : "mlink-s"}`}>
                        {l.label}{i === 0 ? " →" : ""}
                      </a>
                    ))}
                  </div>
        ...
        <div className="dash">
          <div className="dash-nav">
            <div className="dash-brand">
              ...
              <div>
                <h1>Dashboard</h1>
                <p>Managing Rex Magnus portfolio</p>
              </div>
            </div>
          ...
          <div className="dash-body">
            ...
            {adminTab === "analytics" && (
              <div className="acard">
                ...
                <table>
                  <thead>
                    <tr><th>#</th><th>Title</th><th>Genre</th><th>Clicks</th><th>Popularity</th><th>Status</th></tr>
                  </thead>
                  <tbody>
                        ...
                        <tr key={book.id}>
                          ...
                          <td>
                            <div className="bar-row">
                              <div className="bar-t">
                                <div className="bar-f" style={{ width: `${Math.max((c / maxClicks) * 100, 2)}%` }} />
                              </div>
                              <span style={{ fontSize: "0.72rem", color: "var(--muted)", minWidth: "30px" }}>
                                {Math.round((c / maxClicks) * 100)}%
                              </span>
                            </div>
                          </td>
            ...
            {adminTab === "security" && <PasswordForm onPasswordChanged={() => setAdminTab("analytics")} />}
          </div>
        </div>
      )}
...
function CommunityAdmin({ sb }) {
  ...
  useEffect(() => {
    ...
    sb.from("moderation").select("*").order("updated_at", { ascending: false }).then(({ data }) => setUsers(data || []));
  }, []);
  ...
  const moderateUser = async (userId, userName, action) => {
    ...
    const row = { user_id: userId, user_name: userName, status: action, warnings: existing?.warnings || 0, reason: action, updated_at: new Date().toISOString() };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
            ...
            {channels.map((ch) => (
              <div key={ch.id} style={{ display: "flex", alignItems: "center", gap: "0.8rem", padding: "0.8rem 1rem", background: "rgba(255,255,255,0.02)", border: "1px solid var(--border)", borderRadius: "6px" }}>
                ...
                <div style={{ display: "flex", gap: "0.4rem", flexShrink: 0 }}>
                  ...
                  <button className="mdel" onClick={() => deleteChannel(ch.id)}>🗑</button>
                </div>
              </div>
            ))}
            {channels.length === 0 && <p style={{ fontFamily: "'Rajdhani',sans-serif", color: "var(--muted)", textAlign: "center", padding: "2rem" }}>No channels yet.</p>}
          </div>
        </div>
      )}
      ...
      {tab === "users" && (
        <div className="acard">
          <div className="acard-head"><h3>User Moderation</h3><p>Mute or ban users from the community.</p></div>
          {users.length === 0 ? (
            <p style={{ fontFamily: "'Rajdhani',sans-serif", color: "var(--muted)", textAlign: "center", padding: "2rem" }}>No flagged users yet.</p>
          ) : (
            <table>
          ...
          <div className="bform-box">
            ...
            <div className="ff">
              <label style={{ display: "flex", alignItems: "center", gap: "0.7rem", cursor: "pointer" }}>
                <input type="checkbox" checked={form.is_adult} onChange={(e) => sf("is_adult", e.target.checked)} style={{ width: 16, height: 16, accentColor: "var(--magenta)" }} />
            ...
            {form.is_adult && (
              <div style={{ padding: "0.7rem 1rem", background: "rgba(255,45,122,0.06)", border: "1px solid rgba(255,45,122,0.2)", borderRadius: "4px", marginBottom: "0.8rem" }}>
                <p style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: "0.78rem", color: "var(--magenta)", letterSpacing: "0.04em" }}>⚠ 18+ channels are exempt from profanity filtering.</p>
              </div>
            )}
            <div className="facts">
...
function BookForm({ book, onSave, onCancel }) {
  const [f, setF] = useState(
    book || { title: "", genre: "Urban Fantasy", synopsis: "", coverUrl: "", links: [{ label: "", url: "" }] }
  );
      ...
      <div className="bform-box">
        ...
        <div className="ff">
          ...
          <div className="img-upload-wrap">
            ...
            {imgTab === "url" && (
              <input value={f.coverUrl.startsWith("data:") ? "" : f.coverUrl} onChange={(e) => s("coverUrl", e.target.value)} placeholder="https://example.com/cover.jpg" />
            )}
            {imgTab === "upload" && (
...
function PasswordForm({ onPasswordChanged }) {
  ...
  return (
    <div className="security-card">
      ...
      <p className="sec-desc">Update your admin password. You'll need your current password to make changes.</p>
      <div className="ff">
      ...
      <div className="ff">
        ...
        {next && (
          <>
            ...
            <p className="pw-match" style={{ color: s <= 1 ? "var(--magenta)" : s <= 2 ? "var(--gold)" : "#22c55e" }}>{strengthLabel}</p>
          </>
        )}
        <div className="pw-rules">
          <p><span>Tips:</span> Use 8+ characters, a mix of uppercase, numbers, and symbols.</p>
        </div>
      </div>
      <div className="ff">
      ...
      <button className="btn-primary-dash" onClick={handleSave} disabled={success} style={{ opacity: success ? 0.5 : 1, cursor: success ? "default" : "pointer" }}>
        Update Password
      </button>
    </div>
  );
    ...
    <div>
        ...
        <div className="author-photo-editor">
          ...
          <div className="author-photo-controls">
            ...
            <p style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: "0.72rem", color: "var(--muted)", marginTop: "0.5rem", letterSpacing: "0.04em" }}>
              Recommended: square or portrait image, at least 400x500px
            </p>
          </div>
        </div>
      </div>
      <div className="ff">
        ...
        <div className="socials-editor">
          ...
          <button className="add-link" style={{ marginTop: "0.5rem" }} onClick={() => s("socials", [...(f.socials || []), { label: "", url: "" }])}>
            + Add Social Link
          </button>
        </div>
      </div>
